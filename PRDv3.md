...
> Manajemen dokumen referensi untuk AI Bot

| Feature | Description | Status |
|---------|-------------|--------|
| Knowledge Base | Upload, kelola, dan hapus dokumen referensi AI Bot | ✅ Done |
...
| ID | User Story | Acceptance Criteria |
|----|------------|---------------------|
| US-2.1.1 | Sebagai petugas SS, saya ingin melihat daftar mahasiswa berisiko agar bisa melakukan intervensi dini | - Tabel menampilkan nama, NIM, semester, mode, kategori, alfa, level - Badge warna sesuai risk level |
...
| US-2.1.3 | Sebagai petugas SS, saya ingin menghubungi mahasiswa via WhatsApp dengan template pesan | - Button "WA" di setiap baris buka modal pilih template - Pilih template → preview pesan dengan data student ({{name}}, {{nim}}, {{semester}} diganti otomatis) - Klik "Kirim ke WhatsApp" → buka wa.me dengan pesan terisi |
| US-2.1.4 | Sebagai petugas SS, saya ingin melihat rincian absensi per mata kuliah | - Button "Detail" di setiap baris - Modal menampilkan nama, NIM, dan rincian alfa tiap mata kuliah |
...
| ID | User Story | Acceptance Criteria |
|----|------------|---------------------|
| US-3.2.1 | Sebagai petugas SS, saya ingin mengirim broadcast ke mahasiswa tertarget | - Form judul dan isi pesan - Pilih template dari dropdown - Template auto-fill judul dan isi saat dipilih |
| US-3.2.2 | Sebagai petugas SS, saya ingin memilih target berdasarkan atribut atau grup | - Tab "Filter Atribut" dan "By Group" - Estimasi penerima update real-time - Duplikat otomatis dihapus jika pilih multiple grup |
...
| US-3.2.4 | Sebagai petugas SS, saya ingin menyimpan draft broadcast untuk dikirim nanti | - Button "Simpan Draft" → form kosong + otomatis scroll ke panel "Draft Tersimpan" - Draft muncul sebagai card (judul, preview isi, tanggal) - Klik draft → isi form terisi kembali - Button hapus di setiap draft dengan konfirmasi |
...
#### Feature 5.1: Knowledge Base Management

| ID | User Story | Acceptance Criteria |
|----|------------|---------------------|
| US-5.1.1 | Sebagai petugas SS, saya ingin mengupload dokumen referensi untuk AI Bot | - Upload zone dengan drag & drop - Support format PDF, DOCX, XLSX, TXT, CSV - Preview nama dan ukuran file sebelum upload |
| US-5.1.2 | Sebagai petugas SS, saya ingin melihat daftar dokumen yang sudah diupload | - Tabel: Nama Dokumen, Tipe, Ukuran, Tanggal Upload, Status, Aksi - Status dot: Aktif (hijau), Diproses (kuning), Error (merah) |
| US-5.1.3 | Sebagai petugas SS, saya ingin menghapus dokumen yang tidak lagi diperlukan | - Button hapus di setiap baris - Konfirmasi sebelum hapus - Dokumen langsung tidak digunakan AI Bot |
| US-5.1.4 | Sebagai petugas SS, saya ingin melihat status pemrosesan dokumen | - Status "Diproses..." setelah upload - Otomatis berubah jadi "Aktif" setelah selesai diproses (simulasi 3 detik) |
...
5. SS klik Detail untuk lihat rincian absensi per mata kuliah
6. SS klik WA → modal pilih template muncul
7. Pilih template → preview pesan otomatis dengan data student
8. Klik "Kirim ke WhatsApp" → buka wa.me dengan pesan terisi
```

### Flow 3: Broadcast Communication
```
1. SS buat broadcast baru
2. Pilih template (opsional) → auto-fill judul & isi
3. Set target: filter atribut atau pilih grup
4. Lihat estimasi penerima (update real-time)
5. Kirim broadcast, atau simpan draft untuk nanti
```

### Flow 4: Knowledge Base Management
```
1. SS upload dokumen baru (drag & drop atau pilih file)
2. Preview nama dan ukuran file
3. Klik "Upload & Proses"
4. Status "Diproses..." → otomatis berubah jadi "Aktif" setelah 3 detik
5. Dokumen aktif digunakan AI Bot sebagai referensi
6. SS bisa hapus dokumen kapan saja dengan konfirmasi
```
...
{
  id: "DOC001",               // Unique ID
  title: "...",               // Judul dokumen (nama file tanpa ekstensi)
  fileName: "...",            // Nama file asli
  fileType: "pdf",            // pdf | docx | xlsx | txt | csv
  fileSize: "245 KB",         // Ukuran file
  uploadedAt: "2026-06-01",   // Tanggal upload
...
```javascript
{
  ...
  content: "...",             // Isi pesan
  createdAt: "2026-06-01",    // Tanggal draft dibuat
}
```
